...
> 关联：[需求文档](../需求文档.txt) · [比赛与记录](./03-比赛与记录模块.md) · [Rating计算](./06-Rating计算模块.md) · [数据导出与发布](./05-数据导出与发布模块.md) · [前端展示](./08-前端展示模块.md)
...
> ⚠️ 若 [09-Reflex-Web改造提案](./09-Reflex-Web改造提案.md) 被采纳：榜单模式、时间维度、
> 排名规则（§2/§3/§4.3）**全部不变**；但 §4.1 的「每个 mode×period 组合一个预生成 JSON 文件」
> 不再需要 —— 筛选条件变成服务端 state var，切换即重查。见 [12](./12-Web交互与页面-Reflex.md) §2.2。
...
### 4.1 榜单行 `ratings/{mode}/{period_key}.json`
