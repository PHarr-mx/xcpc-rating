...
class TestTeamUpdate:
    ...
    def test_update_no_fields(self, team_service):
        from xcpc_core.team.models import TeamUpdate
...
class TestTeamPersistence:
    def test_persists_to_db(self, temp_team_store, team_service):
        team = team_service.create_team(TeamCreate(members=["p001", "p002"], aliases=["持久化"]))
        reloaded = TeamStore(temp_team_store.session).get(team.id)
        assert reloaded is not None
        assert reloaded.id == team.id
        assert reloaded.member_key == "p001|p002"
        assert reloaded.members == ["p001", "p002"]
        assert reloaded.size == 2
        assert reloaded.aliases == ["持久化"]
